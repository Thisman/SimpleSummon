...
{
    internal sealed class NetworkSignBuilderBoard
    {
        private readonly NetworkList<byte> slots;
        private readonly NetworkVariable<bool> completed;
        private readonly SignBuilderModel offlineModel;

        public NetworkSignBuilderBoard(
            NetworkList<byte> slots,
            NetworkVariable<bool> completed,
            SignBuilderModel offlineModel)
        {
            this.slots = slots;
        ...
        {
            if (index < 0 || index >= SignBuilderState.SlotCount)
            {
                throw new ArgumentOutOfRangeException(nameof(index));
            }

            return isSpawned
                ? index < slots.Count ? slots[index] : SignBuilderState.Empty
                : offlineModel.Slots[index];
        }

        public void Copy(bool isSpawned, byte[] destination)
        {
            if (destination == null || destination.Length != SignBuilderState.SlotCount)
            {
        ...
        {
            byte[] result = new byte[SignBuilderState.SlotCount];
        ...
        public void Publish(bool isSpawned, byte[] board)
        {
            if (isSpawned)
            {
                ...
                completed.Value = SignBuilderState.IsCompleted(board);
            }
            else
            {
        ...
        {
            int count = 0;
            for (int i = 0; i < SignBuilderState.SlotCount; i++)
            {
                if (GetSlot(isSpawned, i) != SignBuilderState.Empty)
                {
                    count++;
                }
            }
