...
namespace SimpleSummon.Network
{
    ...
    public sealed class NetworkSignDrawing : NetworkBehaviour
    {
        ...
        private readonly NetworkVariable<SignDrawingState> state =
            new(SignDrawingState.Available);
        private readonly NetworkVariable<ulong> drawingClientId = new(NoOwner);
        private readonly NetworkList<NetworkSignPoint> points = new();
        private readonly NetworkRateLimiter submitLimiter = new(30d);
        private readonly NetworkRateLimiter eraseLimiter = new(30d);
        private readonly SignDrawingModel model = new();
        private SignDrawingService service;
        private NetworkSignDrawingState drawingState;

        public event Action StateChanged;
        public event Action DrawingChanged;

        public SignDrawingState State => IsSpawned ? state.Value : model.State;
        ...
        private void Awake()
        {
            service = new SignDrawingService(model);
            drawingState = new NetworkSignDrawingState(model, points);
        }
        ...
        public NetworkSignPoint GetPoint(int index) => drawingState.Get(IsSpawned, index);
        ...
        public void SubmitPoints(NetworkSignPoint[] submittedPoints)
        {
            ...
            if (!IsSpawned)
            {
                if (service.Submit(NetworkSignPointMapper.ToCommand(
                        OfflineActorId,
                        submittedPoints)))
                {
        ...
        public void Erase(Vector2 position, float radius)
        {
            if (!IsSpawned)
            {
                if (service.Erase(NetworkSignPointMapper.ToEraseCommand(
                        OfflineActorId,
                        position,
                        radius)))
                {
        ...
        private void SubmitPointsRpc(
            NetworkSignPoint[] submittedPoints,
        ...
        private void EraseOnServer(ulong clientId, Vector2 position, float radius)
        {
            ...
            if (!eraseLimiter.TryAcquire(request) ||
                !service.Erase(NetworkSignPointMapper.ToEraseCommand(
                    clientId,
                    position,
                    radius)))
            {
        ...
        private void HandleStateChanged(SignDrawingState _, SignDrawingState __) =>
            StateChanged?.Invoke();

        private void HandleOwnerChanged(ulong _, ulong __) => StateChanged?.Invoke();
        private void HandlePointsChanged(NetworkListEvent<NetworkSignPoint> _) =>
            DrawingChanged?.Invoke();
    }
}
