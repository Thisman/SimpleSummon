...
namespace SimpleSummon.Runtime
{
    public sealed class CraftingView : MonoBehaviour
    {
        ...
        {
            GameLocalizationController.AddLocaleChangedListener(RefreshText);
        }

        private void OnDisable()
        {
            GameLocalizationController.RemoveLocaleChangedListener(RefreshText);
        }
        ...
        public void SetState(bool crafted, int resources, int requiredResources, bool canCraft)
        {
            statusText.text = crafted
                ? GameLocalizationController.Get("craft.complete")
                : GameLocalizationController.FormatQuestCount("craft.resources", resources, requiredResources);
            craftButton.interactable = !crafted && canCraft;
        }

        private void RefreshText()
        {
            titleText.text = GameLocalizationController.Get("craft.title");
            exitHintText.text = GameLocalizationController.Get("craft.exit_hint");
            craftButton.GetComponentInChildren<TMP_Text>().text = GameLocalizationController.Get("craft.button");
        }
